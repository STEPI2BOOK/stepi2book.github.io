...
staircase2014.EM =function(data,p=1,block=NULL,covariate=NULL,B0=NULL,init=NULL,a=2,r=.5,verbose=F,
     maxit=20,tol=1e-6) {
...
if (b0) {     # *** Mar 2014 **** add this to cover the case when B0 is provided and doesn't need to be estimated
if (dim(Z)[2]> 1) B0 = matrix(apply(B0,1,mean),nrow=q,ncol=dm[K,2])
   else B0 = matrix(B0,nrow=q,ncol=dm[K,2])
        }     # *** Mar 2014 **** add this to cover the case when B0 is provided 
...
B0.tmp = crossprod(beta.tmp,R)
 ...
 else {B0.tmp = B0}  # *** Mar 2014 **** add this to cover the case when B0 is provided
